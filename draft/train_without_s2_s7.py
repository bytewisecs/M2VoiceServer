...
# 排除采集质量可疑的两个位置。
# s2对应原GT 2/12，s7对应原GT 7/17。
EXCLUDED_SUFFIXES = {"s2", "s7"}
ACTIVE_SUFFIX_NUMBERS = [1, 3, 4, 5, 6, 8, 9, 10]
...
def evaluate_detailed(model, loader, device, label_texts):
    ...
    suffix_results = []
    classes_per_command_set = len(ACTIVE_SUFFIX_NUMBERS)

    for compact_index, suffix_number in enumerate(ACTIVE_SUFFIX_NUMBERS):
        label_ids = [
            compact_index,
            compact_index + classes_per_command_set,
        ]
        suffix_correct = sum(
...
def load_label_texts(gt_file):
    ...
    if len(all_label_texts) != 20:
        raise ValueError(
            f"GT文件应包含20条非空句子，当前为{len(all_label_texts)}条："
            f"{gt_file}"
        )

    if len(set(all_label_texts)) != 20:
        raise ValueError("GT文件清洗后存在重复句子。")

    # 原始GT按前10类、后10类排列；两组中的s2和s7均删除。
    label_texts = [
        text
        for original_label_id, text in enumerate(all_label_texts)
        if f"s{original_label_id % 10 + 1}" not in EXCLUDED_SUFFIXES
    ]

    if len(label_texts) != 16:
        raise ValueError(
            f"过滤s2/s7后应剩余16类，当前为{len(label_texts)}类。"
        )
...
def train(modality="fusion"):
    ...
    checkpoint_dir = (
        f"checkpoints_text_cls_split2_v2_without_s2_s7_{modality}"
    )
    ...
    expected_sizes = {"train": 264, "val": 32, "test": 32}
    actual_sizes = {
    ...
    if actual_sizes != expected_sizes:
        raise ValueError(
            f"过滤s2/s7后的样本数量异常。"
            f"期望={expected_sizes}，实际={actual_sizes}"
        )

    print(f"\nExcluded suffixes: {sorted(EXCLUDED_SUFFIXES)}")
    print(f"Filtered split sizes: {actual_sizes}")
...
if __name__ == "__main__":
    # 使用原始Fusion baseline，仅排除s2和s7。
    experiment_modalities = ["fusion"]

    ablation_results = []
    for experiment_modality in experiment_modalities:
        print("\n" + "#" * 90)
        print(f"Start experiment: {experiment_modality}")
        print("#" * 90)
        ablation_results.append(train(experiment_modality))

    ablation_summary_path = "without_s2_s7_summary.csv"
    with open(
